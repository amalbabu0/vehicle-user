...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // This page keeps the full set of tags — title, description, OG, Twitter —
  // because they're what search engines and ordinary link previews read.
  // WhatsApp specifically never reaches this metadata: proxy.ts rewrites its
  // crawler to app/preview/vehicles/[slug], which serves the same URL with a
  // photo-only card, because a lister's WhatsApp message already lists every
  // spec and the card was repeating them. Nothing here should be trimmed for
  // WhatsApp's sake — that's what the preview route is for.
  return {
    title,
    description,
    keywords,
